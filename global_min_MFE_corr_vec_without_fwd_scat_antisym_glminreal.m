%mix_x=0 and min_y nonzero, so start at (min_y,min_y) i.e. along diagonal
...
N_x=80;
N_y=80;
mod_g_real_space=12.3;                                                      %interaction strength
% pi=3.14;
% k_0=1/pi;
delta_x_max=0.3;
delta_y_max=0.3;
...
k_y=(0:(2*pi/N_y):(2*pi/N_y)*(N_y-1))';
k_array=[ repelem(k_x,numel(k_y)) repmat(k_y,numel(k_x),1) ];
delta_k_matrix=min_y*sin(k_array(:,1))-1i*(min_y)*sin(k_array(:,2));
...
hold on
real(MFE_iterations_dimensionless(100))

%----------------------------------------------------------------
%starting close to delta_x=delta_y i.e. (min_y+0.05,min_y+0.05)
...
k_y=(0:(2*pi/N_y):(2*pi/N_y)*(N_y-1))';
k_array=[ repelem(k_x,numel(k_y)) repmat(k_y,numel(k_x),1) ];
delta_k_matrix=(min_y+0.05)*sin(k_array(:,1))-1i*(min_y+0.05)*sin(k_array(:,2));
k_x_axis=0:(2*pi/(N_x)):(2*pi-(2*pi/(N_x)));
k_y_axis=0:(2*pi/(N_y)):(2*pi-(2*pi/(N_y)));
...
%---------------------------------------------------------------------------
%starting on delta_x=0 i.e. (min_x,min_y) because min_x is zero
...
%---------------------------------------------------------------------------
%starting close to delta_x=0 i.e.(min_x+0.05,min_y+0.05) because min_x=0
...
k_y=(0:(2*pi/N_y):(2*pi/N_y)*(N_y-1))';
k_array=[ repelem(k_x,numel(k_y)) repmat(k_y,numel(k_x),1) ];
delta_k_matrix=(min_x+0.05)*sin(k_array(:,1))-1i*(min_y+0.05)*sin(k_array(:,2));
k_x_axis=0:(2*pi/(N_x)):(2*pi-(2*pi/(N_x)));
k_y_axis=0:(2*pi/(N_y)):(2*pi-(2*pi/(N_y)));
...
ylabel('Mean-field energy/Bandwidth');
hold on
var_min_const_line=ones(n_iterations-1,1);
var_min_const_line=var_min_const_line*min_E/(8*t);
plot(no_of_iterations_x_axis,var_min_const_line,'--');
legend('starting delta_x=delta_y','starting around delta_x=delta_y','starting on delta_x=0','starting close to delta_x=0','variational optimum energy')
real(MFE_iterations_dimensionless(100))
